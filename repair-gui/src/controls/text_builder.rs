...
use iced::widget::{Row, button, container, row, space, stack, text};
use iced::{Background, Color, Element, Font, Gradient, Length};
...
impl<Message: Clone> TextBuilder<Message> {
    ...
    pub fn with_bold_typewriter<S: Into<String>>(mut self, text: S) -> Self {
        self.elements.push((
            TextBuilderElement::TypewriterBoldText(text.into()),
            Length::Shrink,
        ));
        self
    }
    ...
    pub fn that_has_width(mut self, width: Length) -> Self {
        if let Some((_, length)) = self.elements.last_mut() {
            *length = width;
        }
        self
    }
        ...
        for (entry, length) in self.elements {
            match entry {
                TextBuilderElement::Text(content) => {
                    elements =
                        elements.push(Self::create_text(content, font, size, wrapping, length));
                }
                TextBuilderElement::BoldText(content) => {
                    elements = elements.push(Self::create_text(
                        content, bold_font, size, wrapping, length,
                    ));
                }
                ...
                TextBuilderElement::TypewriterBoldText(content) => {
                    elements = elements.push(Self::create_text(
                        content,
                        typewriter_bold_font,
                        size,
                        wrapping,
                        length,
                    ));
                ...
                TextBuilderElement::XOutOf { current, total } => {
                    ...
                    elements = elements.push(components.width(length));
                }
